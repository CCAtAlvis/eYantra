...
int main()
{
	...
	int next = 0, coordsLen;
	// coordsLen = number of indexes in coords array
	char orentation = 'n';
	// e: east
	// w: west
	// n: north
	// s: south
	// initially the bot will be at (0,0) with orentation = n
	// i.e. looking (1,0)
	// i havent added the orentiation changing code right now
	// that has to be added after calibrating properly
	...
	while(1)
	{
		...
		if (node condition from sensors == true)
		{
			switch (orentation)
			{
				case 'e':
			...
			if ( currentCoord[0] == coords[next][0] && currentCoord[1] == coords[next][1] )
			{
				next++;
				if (next == coordsLen)
				{
				...
				if (currentCoord[0] < coords[next][0])
				{
					switch (orentation)
					{
						case 'e':
				...
				else if(currentCoord[0] > coords[next][0])
				{
					switch (orentation)
					{
						case 'e':
				...
				{
					// x coords of bot and next position are same,
					// now change the y coords
					if (currentCoord[1] < coords[next][1])
					{
						switch (orentation)
						{
							case 'e':
					...
					else if(currentCoord[1] > coords[next][1])
					{
						switch (orentation)
						{
							case 'e':
